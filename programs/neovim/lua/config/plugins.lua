    ...
    before = function()
      ...
      vim.lsp.config("*", {
        on_attach = function(client, bufnr)
          local bufnr_path = vim.api.nvim_buf_get_name(bufnr)
          local blacklisted_patterns = { "node_modules/", "vendor/", "target/", "build/", "dist/", "%.lock" }
          for _, pattern in ipairs(blacklisted_patterns) do
            if string.find(bufnr_path, pattern) then
              client.stop()
            end
          end
